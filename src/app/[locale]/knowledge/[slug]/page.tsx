 ...
 * "related articles" list before the closing CTA.
 *
  ...
  return (
    <main>
      ...
      <section className="bg-cream px-6 py-16 sm:px-8 sm:py-20">
        ...
        <div className="mx-auto mt-6 flex max-w-xl flex-wrap items-center gap-x-5 gap-y-2 text-xs text-charcoal/50 sm:text-sm">
        ...
        {showToc ? (
          <div className="mx-auto mt-8 max-w-xl">
            <ArticleToc headings={headedSections.map((s) => ({ id: s.anchorId, text: s.heading }))} locale={locale} />
          </div>
        ) : null}
        ...
        {relatedService ? (
          <Reveal className="mx-auto mt-14 max-w-xl">
            <Link
              ...
              className="group flex items-center justify-between gap-4 rounded-2xl border border-gold/25 bg-warm-white px-6 py-6 transition-colors duration-300 ease-out hover:border-gold/50 sm:px-8"
            >
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-gold">{RELATED_SERVICE_LABEL[locale]}</p>
                <p className="mt-1 text-base font-bold text-charcoal sm:text-lg">{relatedService.title[locale]}</p>
              </div>
              <span className="shrink-0 text-sm font-medium text-gold">
                {RELATED_SERVICE_CTA[locale]} {backArrow === "→" ? "←" : "→"}
              </span>
            </Link>
          </Reveal>
        ) : null}
